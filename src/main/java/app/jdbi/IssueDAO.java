...
public interface IssueDAO {
    ...
    /*@SqlUpdate("INSERT INTO issue (project, created_on, issue_title,
    issue_text, created_by, assigned_to, status_text) values ( 
        :project,
        CASE WHEN :created_on THEN :created_on ELSE created_on END,
        CASE WHEN :issue_title THEN :issue_title ELSE issue_title END,
        CASE WHEN :issue_text THEN :issue_text ELSE issue_text END,
        CASE WHEN :created_by THEN :created_by ELSE created_by END,
        CASE WHEN :assigned_to THEN :assigned_to ELSE assigned_to END,
        CASE WHEN :status_text THEN :status_text ELSE status_text END)")
    void insert(@Bind("project") String project, @Bind("created_on") String created_on, 
    @Bind("issue_title") String issue_title, @Bind("issue_text") String issue_text, 
    @Bind("created_by") String created_by, @Bind("assigned_to") String assigned_to, 
    @Bind("status_text") String status_text);
    */
    ...
    @SqlUpdate("UPDATE issue SET project = :project, 
    created_on = CASE WHEN :created_on THEN :created_on ELSE created_on END, 
    issue_title CASE WHEN :issue_title THEN :issue_title ELSE issue_title END,
    issue_text = CASE WHEN :issue_text THEN :issue_text ELSE issue_text END,
    created_by = CASE WHEN :created_by THEN :created_by ELSE created_by END,
    assigned_to = CASE WHEN :assigned_to THEN :assigned_to ELSE assigned_to END,
    status_text = CASE WHEN :status_text THEN :status_text ELSE status_text END
    WHERE _id = :_id")
    void update(@Bind("_id") int _id, @Bind("project") String project, @Bind("created_on") String created_on, 
    @Bind("issue_title") String issue_title, @Bind("issue_text") String issue_text, 
    @Bind("created_by") String created_by, @Bind("assigned_to") String assigned_to, 
    @Bind("status_text") String status_text);
    */
    ...
    */
    @SqlQuery("SELECT * FROM issue WHERE TRUE AND CASE WHEN :_id IS NOT NULL THEN _id = :_id ELSE TRUE END AND CASE WHEN :created_on::timestamp with time zone IS NOT NULL THEN created_on = :created_on::timestamp with time zone ELSE TRUE END AND CASE WHEN :updated_on::timestamp with time zone IS NOT NULL THEN updated_on = :updated_on::timestamp with time zone ELSE TRUE END AND CASE WHEN :issue_title IS NOT NULL THEN issue_title = :issue_title ELSE TRUE END AND CASE WHEN :issue_text IS NOT NULL THEN issue_text = :issue_text ELSE TRUE END AND CASE WHEN :created_by IS NOT NULL THEN created_by = :created_by ELSE TRUE END AND CASE WHEN :assigned_to IS NOT NULL THEN assigned_to = :assigned_to ELSE TRUE END AND CASE WHEN :status_text IS NOT NULL THEN status_text = :status_text ELSE TRUE END")
    List<Issue> find(@Bind("_id") Optional <Integer> _id, @Bind("project") Optional <String> project, 
    @Bind("created_on") Optional <Timestamp> created_on, @Bind("updated_on") Optional <Timestamp> updated_on, 
    ...
    */
    @SqlQuery("DELETE FROM issue WHERE _id = :_id RETURNING *")
    List<Issue> deleteById(@Bind("_id") int _id);
}
