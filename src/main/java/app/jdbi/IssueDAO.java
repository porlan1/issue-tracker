...
public interface IssueDAO {
    ...
    */
    ...
    List <Issue> create(@Bind("project") String project,
    ...
    */
    ...
    SELECT * FROM issue 
    WHERE
    CASE WHEN :_id IS NOT NULL THEN _id = :_id ELSE TRUE END AND 
    CASE WHEN :issue_title IS NOT NULL THEN issue_title = :issue_title ELSE TRUE END AND 
    CASE WHEN :issue_text IS NOT NULL THEN issue_text = :issue_text ELSE TRUE END AND 
    CASE WHEN :created_by IS NOT NULL THEN created_by = :created_by ELSE TRUE END AND 
    CASE WHEN :assigned_to IS NOT NULL THEN assigned_to = :assigned_to ELSE TRUE END AND 
    CASE WHEN :status_text IS NOT NULL THEN status_text = :status_text ELSE TRUE END
    */
    @SqlQuery("SELECT * FROM issue WHERE CASE WHEN :_id IS NOT NULL THEN _id = :_id ELSE TRUE END AND CASE WHEN :project IS NOT NULL THEN project = :project ELSE TRUE END AND CASE WHEN :open IS NOT NULL THEN open = :open ELSE TRUE END AND CASE WHEN :issue_title IS NOT NULL THEN issue_title = :issue_title ELSE TRUE END AND CASE WHEN :issue_text IS NOT NULL THEN issue_text = :issue_text ELSE TRUE END AND CASE WHEN :created_by IS NOT NULL THEN created_by = :created_by ELSE TRUE END AND CASE WHEN :assigned_to IS NOT NULL THEN assigned_to = :assigned_to ELSE TRUE END AND CASE WHEN :status_text IS NOT NULL THEN status_text = :status_text ELSE TRUE END")
    ...
    */
    @SqlUpdate("DELETE FROM issue WHERE _id = :_id AND project = :project")
    Boolean deleteById(@Bind("_id") int _id, @Bind("project") String project);
}
